...
void TLInitialize(TLSequencer** tltest, PluginManager** plugins, std::function<void(TLLocalConfig&)> tlcfgInit)
{
    ...
    tlcfg.masterCountPerCoreTLM         = TLTEST_DEFAULT_MASTER_COUNT_PER_CORE_TLM;
    ...
    tlcfg.memoryReadDepth               = TLTEST_DEFAULT_MEMORY_READ_DEPTH;
    tlcfg.memoryReadLatency             = TLTEST_DEFAULT_MEMORY_READ_LATENCY;
...
#   define INI_OVERRIDE_STRING(section_name, key, target) \
    ...
    INI_OVERRIDE_INT("tltest.config", "memory.backend.rd.depth",    tlcfg.memoryReadDepth);
    INI_OVERRIDE_INT("tltest.config", "memory.backend.rd.latency",  tlcfg.memoryReadLatency);
