...
{
    TLSequencer::CMOPort& port = passive->CMO(deviceId);
    port.req.ready  = ready;
}
...
{
    TLSequencer::CMOPort& port = passive->CMO(deviceId);
    *valid          = port.req.valid;
    *opcode         = port.req.opcode;
    *address        = port.req.address;
}
//
...
{
    TLSequencer::CMOPort& port = passive->CMO(deviceId);
    port.resp.valid     = valid;
    port.resp.address   = address;
}
