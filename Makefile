...
tltest-prepare-all-coupledL2:
	cmake ./main -B $(TLTEST_BUILD_DIR) -DBUILD_DPI=ON -DBUILD_V3=ON $(CMAKE_CXX_COMPILER) \
		-DDUT_PATH="${PWD}/dut/CoupledL2" -DTLTEST_MEMORY=0 $(TLTEST_VERILATED_CMAKE_ARG)

tltest-prepare-dpi-coupledL2:
	cmake ./main -B $(TLTEST_BUILD_DIR) -DBUILD_DPI=ON -DBUILD_V3=OFF $(CMAKE_CXX_COMPILER) \
		-DDUT_PATH="${PWD}/dut/CoupledL2" -DTLTEST_MEMORY=0 $(TLTEST_VERILATED_CMAKE_ARG)

tltest-prepare-v3-coupledL2:
	cmake ./main -B $(TLTEST_BUILD_DIR) -DBUILD_V3=ON -DBUILD_DPI=OFF $(CMAKE_CXX_COMPILER) \
		-DDUT_PATH="${PWD}/dut/CoupledL2" -DTLTEST_MEMORY=0 $(TLTEST_VERILATED_CMAKE_ARG)


tltest-prepare-all-openLLC:
	cmake ./main -B $(TLTEST_BUILD_DIR) -DBUILD_DPI=ON -DBUILD_V3=ON $(CMAKE_CXX_COMPILER) \
		-DDUT_PATH="${PWD}/dut/OpenLLC" $(TLTEST_VERILATED_CMAKE_ARG)
...
coupledL2-verilate:
	rm -rf $(TLTEST_VERILATED_DIR)
	mkdir -p $(TLTEST_VERILATED_DIR)
	verilator --trace-fst --cc --build --lib-create vltdut --Mdir $(TLTEST_VERILATED_DIR) ./dut/CoupledL2/build/*.*v -Wno-fatal \
